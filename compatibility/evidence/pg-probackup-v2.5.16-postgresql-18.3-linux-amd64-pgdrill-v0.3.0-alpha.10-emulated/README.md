# pg_probackup 2.5.16 / PostgreSQL 18.3 / Linux amd64
...
timestamp-PITR drills with pg_probackup 2.5.16 and PostgreSQL 18.3.

The run created a real compressed full STREAM backup after 100 rows, committed
and archived row 101, and proved that latest recovery replayed that
transaction. It then recorded the whole-second timestamp
`2026-07-28T02:39:27Z`, committed and archived row 102 after the target, and
proved that timestamp recovery contained exactly 101 rows.

Native backup and WAL validation, native pg_probackup WAL retrieval,
PostgreSQL startup, readiness, SQL boundary assertions, `pg_amcheck`,
...
performance or RTO evidence. The same-host local filesystem catalog does not
establish remote SSH compatibility. Encryption, incremental backups,
cross-version recovery, sub-second timestamp precision, other PITR targets,
and production operation remain outside this observation. The candidate
archive was not a published or signed release asset.
