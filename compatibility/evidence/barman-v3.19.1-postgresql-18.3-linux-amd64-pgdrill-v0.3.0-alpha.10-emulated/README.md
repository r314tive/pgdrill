# Barman 3.19.1 / PostgreSQL 18.3 / Linux amd64
...
timestamp-PITR drills with Barman 3.19.1 and PostgreSQL 18.3.
...
It then recorded the timestamp `2026-07-28T02:38:46.178232Z`, committed and
...
SQL boundary assertions, `pg_amcheck`, schema-only `pg_dump`, five required
policy verdicts, and owned cleanup all passed. The candidate archive has
SHA-256
...
performance or RTO evidence. The same-host local-rsync repository does not
establish SSH, streaming-backup, or cloud-storage compatibility. Encryption,
incremental backups, cross-version recovery, other PITR targets, and
production operation remain outside this observation. The candidate archive
was not a published or signed release asset.
