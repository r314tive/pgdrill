...
func TestRunPassesWhenPGIsReadySucceeds(t *testing.T) {
	runner := &fakeRunner{result: command.Result{
		Evidence: model.CommandEvidence{
			Path:       "pg_isready",
			Args:       []string{"-d", "postgresql://127.0.0.1:15432/postgres?sslmode=disable"},
	...
	probe := New(Config{Timeout: 1500 * time.Millisecond}, runner)

	report, err := probe.Run(context.Background(), model.RunningPostgres{ConnString: "postgresql://127.0.0.1:15432/postgres?sslmode=disable"})
	if err != nil {
	...
	if got, want := runner.invocation.Args, []string{"-d", "postgresql://127.0.0.1:15432/postgres?sslmode=disable", "-t", "2"}; !reflect.DeepEqual(got, want) {
