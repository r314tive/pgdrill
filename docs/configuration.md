...
| `target.startup_timeout` | `30s` | PostgreSQL postmaster readiness for a local restored target |
| `target.shutdown_timeout` | `10s` | Graceful local PostgreSQL shutdown before forced termination |
...
Local startup returns as soon as `postmaster.pid` reports `ready` or `standby`;
the configured timeout is a deadline, not a fixed delay. The required
post-restore probes remain the database-level recovery proof.
