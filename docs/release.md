...
```sh
make -s release-check VERSION=v0.1.0-alpha.7
```
...
```sh
make -s release-artifacts VERSION=v0.1.0-alpha.7
```

Verify them on Linux or macOS respectively:

```sh
(cd dist && sha256sum -c pgdrill_0.1.0-alpha.7_checksums.txt)
(cd dist && shasum -a 256 -c pgdrill_0.1.0-alpha.7_checksums.txt)
```
